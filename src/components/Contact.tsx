...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-28 px-6" style={{ background: '#ebeee0' }}>
      <div className="max-w-[1000px] mx-auto">
        <SectionHeader kicker="Contact" title="Let's build something" subtitle="Open to DevOps roles, collaborations, and interesting infrastructure challenges." />
        ...
        <motion.div
          ...
          whileInView="visible"
          viewport={viewport}
          className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 max-w-[820px]"
        >
          {links.map(l => (
            <motion.a
              key={l.label}
              ...
              whileHover={{ y: -3 }}
              className="flex items-center gap-3.5 px-5 py-4 rounded-2xl text-[0.9rem] font-medium transition-all"
              style={{ background: '#ffffff', border: '1px solid #dde1d2', color: '#282c20' }}
            >
              <span style={{ color: '#111112' }}>{l.icon}</span>
