...
export default function Contact() {
  ...
  return (
    <section id="contact" className="py-28 px-6" style={{ background: '#111112' }}>
      <div className="max-w-[1000px] mx-auto">
        ...
        <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={viewport} className="mb-8">
          <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full text-[0.82rem] font-semibold" style={{ background: 'rgba(210,255,0,0.12)', border: '1px solid rgba(210,255,0,0.3)', color: '#d2ff00' }}>
        ...
        <div className="grid md:grid-cols-[1fr_280px] gap-8">
            ...
            <div className="card-dark p-7">
              <form onSubmit={handleSubmit} className="space-y-5">
                {(['name', 'email'] as const).map(field => (
                  <div key={field}>
                    <label className="block text-[0.8rem] font-semibold uppercase tracking-wider mb-2" style={{ color: '#d2ff00' }}>{field}</label>
                    <input
                      ...
                      className="w-full text-[0.9rem] px-4 py-3 rounded-xl outline-none transition-all"
                      style={inputStyle}
                ...
                <div>
                  <label className="block text-[0.8rem] font-semibold uppercase tracking-wider mb-2" style={{ color: '#d2ff00' }}>message</label>
                  <textarea
                    required rows={5}
                    value={form.message}
                    onChange={e => setForm(f => ({ ...f, message: e.target.value }))}
                    placeholder="Tell me about the role or project..."
                    className="w-full text-[0.9rem] px-4 py-3 rounded-xl outline-none transition-all resize-none"
                    style={inputStyle}
                    onFocus={e => (e.target.style.borderColor = '#d2ff00')}
                    onBlur={e => (e.target.style.borderColor = 'rgba(255,255,255,0.15)')}
                  />
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={viewport} className="flex flex-col gap-3">
            <p className="text-[0.72rem] font-bold uppercase tracking-[0.22em] mb-1" style={{ color: '#b4b8a5' }}>Direct links</p>
            {links.map(l => (
              <motion.a
                key={l.label}
                variants={fadeUp}
                ...
                whileHover={{ y: -2 }}
                className="flex items-center gap-3 px-4 py-3 rounded-xl text-[0.82rem] font-medium transition-all"
                style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.12)', color: '#ebeee0' }}
              >
                <span style={{ color: '#d2ff00' }}>{l.icon}</span>
