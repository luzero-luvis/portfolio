      ...
      <div className="p-7">
        <div className="flex items-start justify-between gap-4 mb-4 flex-wrap">
          <div className="flex items-baseline gap-4">
            ...
            <span className="text-[0.68rem] font-bold px-3 py-1 rounded-full uppercase tracking-wider" style={{ background: '#d2ff00', color: '#111112' }}>
              {project.badge}
            </span>
          </div>
          <motion.a
            ...
            style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.16)', color: '#ebeee0' }}
          >
            {GH_ICON} GitHub
          </motion.a>
        </div>
        ...
        <ul className="space-y-2 mb-5">
          {project.points.map((point, i) => (
            <li key={i} className="flex gap-2.5 text-[0.86rem] leading-[1.5]" style={{ color: '#b4b8a5' }}>
              <span className="shrink-0 mt-[3px]" style={{ color: '#d2ff00' }}>▸</span>
              {point}
            </li>
          ))}
        </ul>
        ...
        <div className="flex flex-wrap gap-1.5">
          {project.tags.map(tag => (
            <span key={tag} className="text-[0.7rem] font-medium px-2.5 py-1 rounded-full" style={{ background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#b4b8a5' }}>
              {tag}
            </span>
          ))}
        </div>
...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-28 px-6" style={{ background: '#111112' }}>
          ...
          {FILTERS.map(f => (
            <button
              ...
              style={active === f
                ...
                : { background: 'transparent', color: '#b4b8a5', border: '1px solid rgba(255,255,255,0.2)' }}
            >
              {f}
            </button>
          ))}
