...
export default function ProjectPage({ slug }: { slug: string }) {
  ...
  return <article className="container section case-study">
    ...
    <div className="case-heading"><div><div className="eyebrow">{work.category} / {work.kind}</div><h1>{work.title}</h1><p className="lead">{work.summary}</p><div className="tags">{work.stack.map(tool => <span key={tool}>{tool}</span>)}</div>{work.repo ? <a className="button button-dark" href={'https://github.com/' + work.repo} target="_blank" rel="noreferrer">Explore the repository ↗</a> : <a className="button button-dark" href={'mailto:' + profile.email + '?subject=Let%E2%80%99s%20talk%20about%20the%20Hetzner%20platform'}>Discuss this work ↗</a>}</div><Architecture compact variant={featuredWork.indexOf(work)} /></div>
