        ...
        <div className="grid lg:grid-cols-[1fr_360px] gap-8 items-start">
          ...
          <motion.div variants={stagger} initial="hidden" whileInView="visible" viewport={viewport} className="flex flex-col gap-5">
            <motion.div variants={fadeUp} className="card-dark p-6">
              <h3 className="text-[0.72rem] font-bold uppercase tracking-[0.22em] mb-5" style={{ color: '#4398cd' }}>Quick Stats</h3>
              <div className="space-y-3.5">
                {[
                  ...
                  <div key={row.label} className="flex justify-between items-center">
                    <span className="text-[0.88rem]" style={{ color: '#8a8a86' }}>{row.label}</span>
                    <span className="text-[0.88rem] font-bold" style={{ color: '#f4f4f2' }}>{row.value}</span>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div variants={fadeUp} className="card-dark p-6">
              <h3 className="text-[0.72rem] font-bold uppercase tracking-[0.22em] mb-5" style={{ color: '#4398cd' }}>Specializations</h3>
              <div className="flex flex-wrap gap-2">
                {specializations.map(s => (
                  <span
                    key={s}
                    className="text-[0.76rem] font-medium px-3 py-1.5 rounded-sm"
